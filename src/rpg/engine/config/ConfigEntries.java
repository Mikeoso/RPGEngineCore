package rpg.engine.config;

/**
 * @author Daniel Dieckmann aka Mike0so
 * This Class is used to determine the right config path
 */
public class ConfigEntries {

	public static String PLAYER_TEMPLATE_BASE_HEALTH = "Player.Template.base-health";
	public static String PLAYER_TEMPLATE_BASE_HEALTHREGEN = "Player.Template.base-healthRegen";
	public static String PLAYER_TEMPLATE_BASE_DAMAGE = "Player.Templase.base-damage";
	public static String PLAYER_TEMPLATE_BASE_ARMOR = "Player.Templase.base-armor";
	public static String PLAYER_TEMPLATE_BASE_CRIT = "Player.Template.base-crit";
	public static String PLAYER_TEMPLATE_BASE_DODGE = "Player.Template.base-dodge";
	
	public static String ITEM_ATTR_HEALTH_KEYWORD = "Item.Attributes.Health.keyword";
	public static String ITEM_ATTR_DAMAGE_KEYWORD = "Item.Attributes.Damage.keyword";
	public static String ITEM_ATTR_HEALTHREGEN_KEYWORD = "Item.Attributes.Health-Regen.keyword";
	public static String ITEM_ATTR_DODGE_KEYWORD = "Item.Attributes.Dodge.keyword";
	public static String ITEM_ATTR_CRITICAL_KEYWORD = "Item.Attributes.Critical.keyword";
	public static String ITEM_ATTR_ARMOR_KEYWORD = "Item.Attributes.Armor.keyword";
	public static String ITEM_ATTR_EXP_KEYWORD = "Item.Attributes.Exp.keyword";
}
